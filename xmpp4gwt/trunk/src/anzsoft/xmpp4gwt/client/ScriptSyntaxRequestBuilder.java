...
public class ScriptSyntaxRequestBuilder 
{
	...
	public String sendRequest(String body,ScriptSyntaxRequestCallback callback)
	{
		...
		if(sendImpl != null)
			sendImpl.sendRequest(URL.encode(body),url,callbackID,this.timeoutMillis,callback);
		else
			sendRequestImpl(URL.encode(body),url,callbackID,this.timeoutMillis,callback);
		return callbackID;
	}
	...
	private native void sendRequestImpl(String body,String url,String callbackID,int timeOut,ScriptSyntaxRequestCallback callbackHandler)
	/*-{
