...
public class PresenceButton extends MenuButton 
{
	...
	public PresenceButton(final ContactView contactView)
	{
		...
		Session.instance().getPresencePlugin().addPresenceListener(new PresenceListener()
		{
			...
			{
				contactCount = Session.instance().getRosterPlugin().getAllRosterItemCount();
				onlineCount = Session.instance().getPresencePlugin().getOnlineCount();
				setCountText(onlineCount+"/"+contactCount);
			}
		});
		
		Session.instance().getRosterPlugin().addRosterListener(new RosterListener()
		{
	...
	public void setCountText(String t)
	{
		...
		if(isRendered())
		{
			String count =(countText != null && !countText.equals("")) ? "(<strong>"+countText+"</strong>)" : "";
