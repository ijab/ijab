...
public class ContactView extends ContentPanel implements PresenceListener
{
    ...
    public enum Status{STATUS_OFFLINE,STATUS_DND,STATUS_XA,STATUS_AWAY,STATUS_ONLINE,STATUS_CHAT};
    public enum RenderMode{All,Online,None};
	...
	private void initView()
	{
		...
		rosterLoader = new BasePagingLoader<PagingLoadResult<ModelData>>(rosterProxy,reader);
    	...
    	aliasColumnConfig.setRenderer(new GridCellRenderer<ModelData>()
    	{
					...
					ListStore<ModelData> store, Grid<ModelData> grid) 
			{
    			...
    			p.add(statusText);
    			return Format.substitute("<span style=\"vertical-align: middle;color:black;font-size:12px;\">{0}</span><span style=\"vertical-align: middle;color:gray;font-size:11px;\">{1}</span>", 
    					p);
			}
    	...
    	*/
    	...
    	grid.addListener(Events.RowClick, new Listener<GridEvent<ModelData> >()
    	{
			public void handleEvent(GridEvent<ModelData> be) 
			{
				...
				ModelData data = datas.get(be.getRowIndex()+viewIndex-viewOffset);
				String jid = data.get(JIDID);
				fireOnOpenChat(JID.fromString(jid));
			}
    	...
    	view.addListener(Events.LiveGridViewUpdate, new Listener<LiveGridEvent<ModelData>>()
    	{
	...
	private String iconFromStatus(final Presence item)
    {
		...
		if(item!=null&&plugin.isAvailableByBareJid(item.getFrom().toStringBare()))
		{
			switch(item.getShow())
			{
    		case dnd:
	...
	public void onInitRoster()
	{
		...
		if(con != null)
			con.suspend();
		rosterProxy.setData(Session.instance().getRosterPlugin().getRosterPacket());
		rosterLoader.load();
		if(con != null)
			con.resume();
		doRefresh();
	...
	private ModelData ensureContactData(final String bareJid)
	{
		...
		if(data == null)
		{
			RosterItem rosterItem = Session.instance().getRosterPlugin().getRosterItem(JID.fromString(bareJid));
			if(rosterItem != null)
			{
	...
	{
		ModelData data = ensureContactData(presenceItem.getFrom().toStringBare());
		if(data!=null)
		{
